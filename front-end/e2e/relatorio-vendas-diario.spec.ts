...
test.describe('Relatório de Vendas por Dia', () => {
  test('Deve gerar o gráfico corretamente ao submeter o filtro com as datas que possuem vendas', async ({ page }) => {
    await page.goto('http://localhost:5173/relatorio-vendas/diario');

    await expect(page.locator('h1')).toHaveText('Relatório de Vendas por Dia');
